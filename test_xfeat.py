    ...
    def match_and_draw(self, ref_frame, current_frame):
        ...
        if len(points1) > 10 and len(points2) > 10:
            ...
            if self.use_homography:
                ...
                feats1 = descs1[idx0].cpu().numpy()
                feats2 = descs2[idx1].cpu().numpy()
                mkpts_0 = kpts1[idx0].cpu().numpy()
                mkpts_1 = kpts2[idx1].cpu().numpy()
                ...
                # feats1 =  feats1.to_tensor()
                # feats2 =  feats2.to_tensor()
                offsets = self.xfeat.net.fine_matcher(torch.cat([feats1, feats2],dim=-1))
                conf = F.softmax(offsets*3, dim=-1).max(dim=-1)[0]
                offsets = self.xfeat.subpix_softmax2d(offsets.view(-1,8,8))
                ...
                kp1 = [cv2.KeyPoint(p[0],p[1], 5) for p in mkpts_0[mask_good]]
                kp2 = [cv2.KeyPoint(p[0],p[1], 5) for p in mkpts_1[mask_good]]
                # cast all mkpts_0 to int
                good_matches = [cv2.DMatch(i,i,0) for i in range(len(kp1))]
